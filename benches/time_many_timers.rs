...
///
/// ## Benchmark Results (Measuring poll+drop with pre-allocated futures)
///
/// ### Master (global timer wheel with mutex)
///
/// ```text
/// Single-threaded: 33.9ms (median)
/// Multi-threaded (8 workers): 22.0ms (median)
/// Speedup: 1.54x
///
/// Contention indicators:
/// - 10 high severe outliers (75-90ms, 3-4x slower than median)
/// - Task duration range: typically 1-6ms, spikes to 20ms on slow runs
/// - Wall clock variance suggests severe mutex contention
/// ```
///
/// ### Current (per-worker timers)
///
/// ```text
/// Single-threaded: 35.0ms (+3.2%, not expected to be meaningful)
/// Multi-threaded (8 workers): 18.8ms
/// Speedup: 1.86x
///
/// Known issues:
/// - Timer HashMap scanned on every worker tick
/// ```
